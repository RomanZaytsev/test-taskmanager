		...
		<div>
			<div class="well clearfix">
				<div class="pull-right">
					<button type="button" class="btn btn-xs btn-primary" id="command-add" data-row-id="0"><span class="glyphicon glyphicon-plus"></span> Создать новую задачу</button>
				</div>
			</div>
			<table id="Task_grid" class="table table-condensed table-hover table-striped" width="60%" cellspacing="0" data-toggle="bootgrid">
				<thead>
					<tr>
...
<div id="add_model" class="modal fade">
	<div class="modal-dialog">
		<div class="modal-content">
			<form id="frm_add" method="post" data-ajaxaction="/task/post" data-event="submit:ajaxFormSubmit,error:formError,success:formSuccess">
...
<div id="edit_model" class="modal fade">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<form id="frm_edit" method="post" data-ajaxaction="/task/update" data-event="submit:ajaxFormSubmit,error:formError,success:formSuccess">
				<div class="modal-body">
						<input type="hidden" id="edit_id" name="id" value="0">
						<div class="form-group">
							<label for="username" class="control-label">имя пользователя:</label>
							<input <?php echo(@$user->name != "admin" ? "disabled" : ""); ?> type="text" class="form-control" id="edit_username" name="username" />
						</div>
						<div class="form-group">
							<label for="email" class="control-label">email:</label>
							<input <?php echo(@$user->name != "admin" ? "disabled" : ""); ?> type="text" class="form-control" id="edit_email" name="email" />
						</div>
						<div class="form-group">
							<label for="text" class="control-label">текст задачи:</label>
							<textarea <?php echo(@$user->name != "admin" ? "disabled" : ""); ?> class="form-control" id="edit_text" name="text"></textarea>
						</div>
						<?php if(@$user->name == "admin"): ?>
						<div class="form-group">
							<label for="status" class="control-label">Выполнена:</label>
							<input type="checkbox" class="form-control" name="done" value="0" style="display:none;" checked="checked" />
  ...
  grid = $("#Task_grid").bootgrid({
    ...
    grid.find(".command-edit").on("click", function(e) {
      ...
      if (conf) {
        ...
        //$("#Task_grid").bootgrid('remove', $(this).data("row-id"))
      }
    });
  });
...
function formSuccess(e) {
	...
	$('#Task_grid').bootgrid('reload');
	//$('.pagination li.last a[href=#last]').click();
	setTimeout(function(){$('#success_model').modal('hide');}, 2500);
}
</script>
<style>
.actionBar .search.form-group {
	display:none;
}
</style>
